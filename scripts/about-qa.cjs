...
(async () => {
  ...
  for (const width of [320,390,768,1440]) {
    ...
    assert(await page.locator('#about .artefacts').evaluate(e=>e.classList.contains('is-revealed')));
    await page.reload();
  ...
  await page.waitForTimeout(2200);
  assert.equal(await page.locator('#about .artefacts').evaluate(e=>e.classList.contains('is-revealed')),false);
  ...
  assert.equal(motion.duration,'0s');assert(['none','matrix(1, 0, 0, 1, 0, 0)'].includes(motion.transform));
  await page.close();
